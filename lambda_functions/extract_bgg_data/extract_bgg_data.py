...
def fetch_game_data(game_id: int, bearer_token: str = None) -> Dict[str, Any]:
    """
    Fetch game data from BoardGameGeek XML API2.
    ...
    """
    url = f"https://api.boardgamegeek.com/xmlapi2/thing?id={game_id}&stats=1"
    ...
    try:
        logger.info(f"Fetching data for game ID: {game_id}")
...
def parse_game_xml(xml_content: str, game_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if item is None:
            logger.warning(f'Non item found for game {game_id}')
            return None
